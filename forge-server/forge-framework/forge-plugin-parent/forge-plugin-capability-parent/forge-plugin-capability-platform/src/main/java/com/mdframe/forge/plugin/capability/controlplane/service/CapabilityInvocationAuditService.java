...
public class CapabilityInvocationAuditService {
    ...
    private static final Pattern SENSITIVE_PAIR = Pattern.compile(
            "(?i)(authorization|token|secret|password|private[_ -]?key|signing[_ -]?key|signature)\\s*[:=]\\s*[^,;\\s]+");
    ...
    private AiCapabilityInvocationLog buildLog(Long tenantId, CapabilityInvocationAuditEvent event) {
        ...
        log.setFailureStage(event.failureStage());
        log.setErrorMessage(sanitizeErrorMessage(event.errorMessage()));
    ...
    private void validateEvent(CapabilityInvocationAuditEvent event) {
        ...
        requireStableCode("失败阶段", event.failureStage(), true);
    ...
    public Page<CapabilityInvocationDetailVO> page(
            Long tenantId,
            PageQuery pageQuery,
            Long clientId,
            String requestId,
            String capabilityCode,
            String capabilityKeyword,
            String actorKeyword,
            String resultCode) {
        Long trustedTenantId = requireTenant(tenantId);
        String normalizedActorKeyword = normalizeKeyword(actorKeyword);
        return executeInTrustedTenant(trustedTenantId, () -> logMapper.selectPage(
                pageQuery.toPage(), trustedTenantId, clientId,
                normalizeKeyword(requestId), normalizeKeyword(capabilityCode),
                normalizeKeyword(capabilityKeyword), normalizedActorKeyword,
                parsePositiveLong(normalizedActorKeyword), normalizeKeyword(resultCode)));
    }

    public CapabilityInvocationDetailVO detail(Long tenantId, Long id) {
        Long trustedTenantId = requireTenant(tenantId);
        if (id == null || id <= 0) {
            throw new BusinessException("调用日志ID无效");
        }
        CapabilityInvocationDetailVO detail = executeInTrustedTenant(
                trustedTenantId, () -> logMapper.selectDetail(trustedTenantId, id));
        if (detail == null) {
            throw new BusinessException("调用日志不存在或无权访问");
        }
        return detail;
    }
    ...
    private String sanitizeErrorMessage(String message) {
        ...
        sanitized = SENSITIVE_PAIR.matcher(sanitized).replaceAll("$1=<REDACTED>");
