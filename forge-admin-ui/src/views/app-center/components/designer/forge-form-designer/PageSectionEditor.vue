  ...
  <div class="page-section-editor">
    ...
    <section class="bottom-bar-editor">
      ...
      <draggable
        ...
        :animation="160"
        class="bottom-action-list"
        @update:model-value="replaceBottomActions"
      >
        <template #item="{ element, index }">
...
const bottomActions = computed(() => draft.value.bottomBar.actions)
...
function replaceBottomActions(actions) {
  draft.value = { ...draft.value, bottomBar: { ...draft.value.bottomBar, actions } }
}
...
function addBottomAction(type) {
  ...
  replaceBottomActions([...bottomActions.value, { ...action, __editorKey: action.actionId }])
}

function patchBottomAction(index, patch = {}) {
  replaceBottomActions(bottomActions.value.map((action, actionIndex) => actionIndex === index ? { ...action, ...patch } : action))
}
...
function removeBottomAction(index) {
  replaceBottomActions(bottomActions.value.filter((_, actionIndex) => actionIndex !== index))
}
