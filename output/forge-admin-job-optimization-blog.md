...
项目地址（README 里有演示环境和文档）：欢迎在评论区交流你遇到过的定时任务事故，我挑典型的在下一篇展开讲。
...
- **标题建议**：`2.8 万行代码重构定时任务模块：从 Cron 劝退到 SSRF 防护，我踩过的坑都在这了`
- **分类**：后端 / Java；**标签**：Java、Spring Boot、Quartz、SSRF、开源
- 掘金支持 Markdown 直接粘贴；代码块语法高亮保留。
- 开头第一段（"几乎每个中后台系统都有定时任务模块……"）会被抓做摘要，保持原样。

## 今日头条发布要点

- **标题建议**（头条标题偏口语、偏悬念）：`一个定时任务模块，为什么值得写 10 个版本？聊聊企业级调度系统的可靠性设计`
- 头条编辑器对 Markdown 支持弱，建议：
  - 把正文粘贴后手动调整标题层级（一、二、三……改为加粗居中或小标题样式）；
  - 代码块转为截图或简化为要点（头条读者技术密度承受力低于掘金，可考虑只保留第一段"反面代码"示例，其余代码块改为文字描述）；
  - 每个"一句话总结"引用块保留，是头条读者喜欢的金句格式。

## 截图清单（共 4 张，按优先级排序）

| 序号 | 截图内容 | 位置 | 优先级 |
|------|----------|------|--------|
| 1 | 任务配置工作台全貌（右侧配置摘要 + 未来 5 次触发时间预览） | 第二章末尾 | 必截 |
| 2 | 日志列表 + 抽屉详情 / 近 24h 监控摘要 | 第四章末尾 | 必截 |
| 3 | API Token 工作台 + cURL 调用示例 | 第六章末尾 | 必截 |
| 4 | 版本路线图或测试通过记录 | 第八章末尾 | 可选 |

## 敏感信息自查（发布前）

- [ ] 截图中如出现 Token 明文、服务器 IP、内网域名，打码或打码后重截；
- [ ] 文中未包含任何真实密钥/连接串（已确认，仅有 `169.254.169.254` 等公开技术地址）；
- [ ] spec 中标注的"真实 MySQL/Redis/Quartz/Flow E2E 为用户侧验收项"——正文已避免宣称完成生产级端到端验证，保持现状即可。
