...
## 178. 自动密钥文件不能用排他锁包住完整只读路径

**发现日期**：2026-08-17

多个独立 JVM（如 Admin 与 Flow）共享同一 `crypto.properties` 时，如果启动引导把“读取已有文件”和“首次生成/兼容升级”都放在 `FileChannel.lock()` 排他锁内，一个服务启动会阻塞另一个服务读取，表现为锁文件被占用。反过来把整段直接改成共享锁，又会让首次生成和缺字段写回失去单写保护。

处理原则：完整文件只在共享锁内读取和校验；文件不存在或缺少允许自动补齐的字段时，释放共享锁后获取排他锁，并在排他锁内重新读取状态再生成或原子写回。跨进程问题必须用独立 JVM 回归测试，不能只依赖同 JVM 线程测试，因为 JVM 内锁或 `OverlappingFileLockException` 会掩盖真实文件锁语义。
