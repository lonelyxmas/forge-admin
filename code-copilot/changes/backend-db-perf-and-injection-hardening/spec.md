...
- [ ] **Q1**：F9 AI 统计优化采用"冗余字段 + 触发器"还是"物化汇总表"还是"定时任务刷新"？影响是否需要 Flyway。
- [ ] **Q2**：F10 待办候选人是否接受新建关联表 `sys_flow_task_candidate`？涉及存量数据迁移。
- [ ] **Q3**：F2 消息发送拆分事务后，发送失败的消息是否需要自动重试？还是仅标记状态由人工处理？
- [ ] **Q4**：F18 LambdaQueryWrapper 迁移是否本轮全量执行？还是按 system -> flow -> message 分批排期？
- [ ] **Q5**：本提案是否拆分为多个独立变更（按 P0/P1/P2/P3）分别执行，还是作为单一大变更分 Task 推进？
