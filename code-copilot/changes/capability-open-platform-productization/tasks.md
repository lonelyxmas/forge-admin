...
## Task 26: 客户端接入工作台与契约易用性优化 🚧

- 能力目录增加受限编辑和安全逻辑删除；启用状态或有效授权存在时阻断删除，保留版本与调用审计。
- 授权候选项与调用指南从已发布 Schema 返回中文字段元数据，统一字段名称、编码、类型、必填性、说明和示例。
- 调用指南重排为四步，示例收敛为 Curl/Java；在线测试复用当前 SPA 内存中的一次性凭据，无凭据时提供客户端凭据引导。
- 客户端页重构为工作台，聚合概览与凭据、能力授权、外围用户映射和调用日志；所有子页面按客户端过滤。
- 调用日志增加安全详情、失败阶段、错误摘要与真实调用用户信息，不保存请求报文和认证敏感信息。
- 外围用户映射增加分页与搜索，并提供管理员显式启用的“已验签手机号唯一匹配”规则；预绑定继续作为默认失败关闭策略。
- 用户继续自行执行 Maven、pnpm、服务、Flyway、接口和浏览器测试；本轮只做差异和冲突标记静态检查。
