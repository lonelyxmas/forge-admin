...
- [ ] 功能 27：能力目录支持修改可变展示信息和安全逻辑删除；已发布版本契约、能力编码和来源绑定继续不可变。
- [ ] 功能 28：调用指南按“选择客户端 → 接入检查 → 请求/响应契约 → 在线测试”分区，调用示例只保留 Curl 与 Java 两类入口。
- [ ] 功能 29：注册、授权和调用指南统一使用字段中文名称、字段编码、类型、必填性、说明和示例元数据。
- [ ] 功能 30：客户端、凭据、授权、外围用户映射和调用日志聚合为客户端工作台，原独立入口保留兼容但不再作为主操作路径。
- [ ] 功能 31：调用日志提供安全详情，明确客户端、真实调用用户、组织、失败阶段、错误码、字段路径和脱敏错误说明。
- [ ] 功能 32：外围用户映射支持服务端分页和搜索；客户端签名断言提供管理员显式启用的可信手机号自动匹配规则。
...
42. 能力编辑仅允许修改名称、描述和可见性等目录展示元数据；能力编码、协议工具名、来源、行为、风险等级、调用主体和已发布版本契约不得通过编辑入口修改。
43. 删除能力必须使用逻辑删除。能力仍启用或存在有效授权时必须阻断并给出处理建议；历史版本和调用日志继续保留用于审计，删除后开放网关不得再解析该能力。
44. 字段选择器不得只展示技术字段编码。服务端必须从实际授权版本 Schema 投影 `fieldCode/fieldLabel/description/type/required`，前端以中文名称为主、字段编码为次要信息。
45. 浏览器和服务端都不得反查已经展示过的 Client Secret、Signing Key 或用户断言私钥。创建或轮换后可在当前 SPA 内存会话自动带入在线测试；无会话凭据时必须引导用户到客户端凭据页轮换或生成，不能展示虚假的“自动获取”。
46. 客户端工作台以单个客户端为上下文展示概览与凭据、能力授权、用户映射、调用日志；所有子查询必须固定携带当前客户端 ID，避免管理员误操作其它客户端。
47. 调用日志错误详情只保存和展示经过长度限制与敏感信息清洗的业务错误摘要；不得记录请求/响应 Body、Token、Secret、私钥、签名或数据库异常中的连接信息。调用人展示真实 `actorUserId`，并关联当前租户用户目录返回用户名和姓名。
48. 客户端签名用户断言默认保持预绑定失败关闭。手机号自动匹配只在管理员为具体客户端显式启用后生效，手机号必须来自该客户端私钥验签通过的短期断言、在租户内唯一匹配普通启用用户；无匹配、多匹配、管理员用户或目录异常均拒绝换取 Token，并在首次成功后固化安全映射。
...
| 新增 | `/ai/capability/update/:id` | POST | 仅修改能力名称、描述和可见性 |
| 新增 | `/ai/capability/delete/:id` | DELETE | 停用且无有效授权时逻辑删除能力 |
| 新增 | `/ai/capability/invocation/:id` | GET | 查询单条安全调用日志详情及调用人信息 |
| 新增 | `/ai/capability/client/:id/user-assertion/mapping/page` | GET | 按客户端分页查询外围用户映射 |
| 新增 | `/ai/capability/client/:id/user-assertion/mapping-rule` | POST | 显式配置预绑定或可信手机号自动匹配规则 |
...
26. 已注册能力可修改名称、描述、可见性；停用并撤销有效授权后可删除，页面明确说明不可修改的契约字段及删除阻断原因。
27. 调用与测试页按四步分区，示例仅保留 Curl/Java；认证方式在示例内部切换，不再拆成多个含义相近的标签。
28. 请求和响应契约以表格展示中文名称、字段编码、类型、必填、说明和示例；注册与授权的字段选择器也使用相同中文元数据。
29. 新建/轮换客户端凭据后，从当前浏览器内存进入调用测试时自动填入；刷新或离开会话后不再可恢复，并提供“去客户端凭据”操作引导。
30. 客户端列表进入工作台后，可在同一上下文维护凭据、授权和用户映射，并查看只属于该客户端的调用日志及安全错误详情。
31. 外围用户映射可分页、按外围标识提示或 Forge 用户检索；用户可选预绑定，或显式启用“已验签手机号唯一匹配”规则，页面解释信任边界和失败条件。
